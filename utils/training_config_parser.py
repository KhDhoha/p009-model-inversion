...
class TrainingConfigParser:
    ...
    def create_datasets(self):
        ...
        data_transformation_test = self.create_transformations(
            mode='test', normalize=True
        )

        # -------------------------
        # Load datasets (base sets)
        # -------------------------
        if name == 'facescrub':
            group = dataset_config.get('facescrub_group', 'all')
            ...
            # Full dataset WITHOUT transform (so subsets can apply different ones)
            from torchvision.datasets import ImageFolder as _ImageFolder

            class ImageFolder(_ImageFolder):
                def find_classes(self, directory):
                    classes, class_to_idx = super().find_classes(directory)
                    # Filter out __pycache__ and any other non-class dirs
                    classes = [c for c in classes if not c.startswith("__")]
                    class_to_idx = {c: class_to_idx[c] for c in classes}
            ...
            # Optional: treat same folder as test set (no split logic dependency)
            ...
            raise Exception(
                f"{name} is not a valid dataset. Please use one of "
                "['facescrub', 'celeba_identities', "
                "'stanford_dogs_uncropped', 'stanford_dogs_cropped', 'custom']."
            )
        ...
        if validation_set_size + train_set_size > len(train_set):
            print(
                "Specified training and validation sets are larger than full dataset.\n"
        ...
        valid_len = len(valid_set) if valid_set else 0
        test_len = len(test_set) if test_set else 0

        print(
            f"Created {name} datasets with "
            f"{train_len:,} training, {valid_len:,} validation and {test_len:,} test samples.\n",
            f"Transformations during training: {train_set.transform}\n",
            f"Transformations during evaluation: {test_set.transform if test_set else None}"
        )
